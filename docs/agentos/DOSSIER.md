...
- F02/F03 integration passed 146 tests and an offline Mac wheel install/update/rollback with a synthetic overlay and preserved user-tree hashes. F04 now passes 153 tests, the public-tree validator, a fresh Mac source install, a Mac offline install/update/rollback fixture, and Linux arm64 wheel smoke. MCP, CLI and project event/task contracts are packaged; human, CLI and MCP versions agree at `0.5.0-beta.1` while distribution metadata is `0.5.0b1`. Actual native hook activation, Windows execution and release read-back remain outstanding.
