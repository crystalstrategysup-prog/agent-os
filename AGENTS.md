...
- Questions, searches, read-only audits and API discovery proceed directly under existing read authority, including unregistered directories. Do not require project init/intake/observe, answers files, active profiles or closeout for them.
- Keep inherited project AGENTS.md as a concise routing index: scope, universal limits and links to the relevant project documents or skills. Read detailed knowledge when the task needs it; verify dated status against current evidence. Length is advisory, never a gate.
- Actual project changes require current dossier, roadmap, stage, architecture and contracts before product writes. Reuse verified facts; ask only material unknowns. Use explicit entry, documentation readiness, approved checks and close/checkpoint under docs/PROCESS.md. Pre-entry failure has no task to close.
...
- Keep the public foundation and versioned user overlay physically separate as defined in `docs/FOUNDATION_OVERLAY.md`. Native hooks are excluded: never install, enable or restore them, including rollback. Explicit CLI gates are not an OS sandbox. An AgentOS CLI or MCP plan does not itself grant external mutation authority.
